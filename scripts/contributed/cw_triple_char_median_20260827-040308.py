"""Combine triples of known basenames by position-wise alphabetic median."""
import os, sys
    ...
    for _, b in seeds:
        for _, c in seeds:
            ...
            for i in range(max(len(a), len(b), len(c))):
                ...
                letters = [x for x in vals if x.isalpha()]
                out.append(sorted(letters)[len(letters)//2] if letters else vals[0])
