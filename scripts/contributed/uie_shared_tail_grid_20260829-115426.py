"""Fill the measured uie family grid using tails shared by multiple axes."""
import collections
import pathlib
import sys
...
def main():
    ...
    candidates = sorted({f"uie_{axis}_{tail}" for axis in axes for tail in tails} - have)
    print(f"uie shared-tail grid: {len(axes):,} axes x {len(tails):,} tails = "
