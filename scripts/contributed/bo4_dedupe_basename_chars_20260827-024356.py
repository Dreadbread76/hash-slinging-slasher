"""Deduplicate basename characters, retaining first occurrence, for BO4 non-sound assets."""
import sys
...
for name in known:
    ...
    if len(base) < 6 or "." in base:
        continue
    seen = set()
    reduced = "".join(ch for ch in base if not (ch in seen or seen.add(ch)))
    out.add((directory + "/" if directory else "") + reduced)
