"""Fill the measured p9 family grid using tails shared by multiple axes."""
import collections
import pathlib
import sys
...
def main():
    ...
    candidates = sorted({f"p9_{axis}_{tail}" for axis in axes for tail in tails} - have)
    print(f"p9 shared-tail grid: {len(axes):,} axes x {len(tails):,} tails = "
