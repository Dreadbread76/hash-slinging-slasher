"""Combine paired basenames by selecting the smaller character at each position."""
import os, sys
...
for d, a in seeds:
    for _, b in seeds:
        out = "".join(a[i] if i >= len(b) else (b[i] if i >= len(a) else min(a[i], b[i])) for i in range(max(len(a), len(b))))
        candidate = d + "/" + out
