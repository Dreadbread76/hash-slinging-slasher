"""Fill the measured ui family using tails shared by multiple observed axes."""
import collections
import pathlib
import sys
...
def main():
    ...
    candidates = sorted({f"ui_{axis}_{tail}" for axis in axes for tail in tails} - have)
    print(f"ui shared-tail grid: {len(axes):,} axes x {len(tails):,} tails = "
