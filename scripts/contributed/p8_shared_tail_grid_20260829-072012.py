"""Fill only the measured p8 family grid, using tails shared by multiple axes."""
import collections
import pathlib
import sys
...
def main():
    ...
    for name in have:
        ...
        if axis and tail:
            pairs[head].append((axis, tail))
    observed = pairs.get("p8", [])
    ...
    candidates = sorted(
        {f"p8_{axis}_{tail}" for axis in axes for tail in tails} - have
    )
    print(f"p8 shared-tail grid: {len(axes):,} axes x {len(tails):,} tails = "
