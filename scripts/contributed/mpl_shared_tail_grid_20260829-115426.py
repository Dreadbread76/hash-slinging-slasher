"""Fill the measured mpl family grid using tails shared by multiple axes."""
import collections
import pathlib
import sys
...
def main():
    ...
    candidates = sorted({f"mpl_{axis}_{tail}" for axis in axes for tail in tails} - have)
    print(f"mpl shared-tail grid: {len(axes):,} axes x {len(tails):,} tails = "
