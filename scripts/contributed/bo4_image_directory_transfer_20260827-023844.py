"""Transfer observed image basenames across observed image directories."""
import sys
from pathlib import Path
ROOT = Path(__file__).resolve().parent.parent
sys.path.insert(0, str(ROOT / "scripts"))
import snapshot
names = {n.strip().lower().replace("\\", "/") for n in snapshot.table_names("fnv1a_ximages") if n.strip()}
names.update(n.strip().lower().replace("\\", "/") for n in snapshot.confirmed_names(kind="image") if n.strip())
pairs = []
