"""Fill the measured p7 family using tails shared by multiple observed axes."""
import collections
import pathlib
import sys
...
def main():
    ...
    candidates = sorted({f"p7_{axis}_{tail}" for axis in axes for tail in tails} - have)
    print(f"p7 shared-tail grid: {len(axes):,} axes x {len(tails):,} tails = "
