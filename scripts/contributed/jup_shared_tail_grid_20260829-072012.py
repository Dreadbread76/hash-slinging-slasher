"""Fill the measured jup family using tails shared by multiple observed axes."""
import collections
import pathlib
import sys
...
def main():
    ...
    candidates = sorted({f"jup_{axis}_{tail}" for axis in axes for tail in tails} - have)
    print(f"jup shared-tail grid: {len(axes):,} axes x {len(tails):,} tails = "
