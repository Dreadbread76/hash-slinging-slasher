...
for name in known:
    ...
    parts = base.split("_")
    if len(parts) < 5 or "." in base:
        continue
    positions = list(range(1, len(parts) - 1, 2))
    values = sorted(parts[i] for i in positions)
    changed = parts[:]
    for i, value in zip(positions, values): changed[i] = value
