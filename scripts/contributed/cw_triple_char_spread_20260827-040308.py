"""Combine triples by mapping alphabetic positions to their min/max spread."""
import os, sys
    ...
    for _, b in seeds:
        for _, c in seeds:
            ...
            for i in range(max(len(a), len(b), len(c))):
                ...
                letters = [ord(v) - 97 for v in vals if v.isalpha()]
                if len(letters) == 3:
                    out.append(chr(97 + (max(letters) - min(letters))))
                else:
                    out.append(vals[0])
