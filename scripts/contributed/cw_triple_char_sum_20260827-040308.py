"""Combine triples of known basenames by modular alphabet-index sum."""
import os, sys
    ...
    for _, b in seeds:
        for _, c in seeds:
            ...
            for i in range(max(len(a), len(b), len(c))):
                ...
                letters = [v for v in vals if v.isalpha()]
                if len(letters) == 3:
                    out.append(chr(97 + sum(ord(v) - 97 for v in letters) % 26))
                else:
                    out.append(vals[0])
