"""Fill the measured emblems family grid using tails shared by multiple axes."""
import collections
import pathlib
import sys
...
def main():
    ...
    for name in have:
        ...
        if head == "emblems" and axis and tail:
    ...
    candidates = sorted({f"emblems_{axis}_{tail}" for axis in axes for tail in tails} - have)
    print(f"emblems shared-tail grid: {len(axes):,} axes x {len(tails):,} tails = "
