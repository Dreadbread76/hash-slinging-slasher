"""Fill only att_ cells whose tail is already used by multiple observed axes."""
import collections
import os
import sys
...
def main():
    ...
    for name in known:
        if not name.startswith("att_") or "/" in name or "." in name:
            continue
    ...
    out = sorted(f"att_{axis}_{tail}" for axis in axes for tail in shared
                 if f"att_{axis}_{tail}" not in known)
    for name in out:
        print(name)
    print("att shared-tail grid: %d axes x %d shared tails = %d unseen cells" %
