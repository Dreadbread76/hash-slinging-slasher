"""Delete one interior basename character from known BO4 non-sound assets."""
import sys
...
for name in known:
    ...
    if "." in base or len(base) < 8:
        continue
    for index in range(1, len(base) - 1):
        if base[index] == "_":
            continue
        out.add((directory + "/" if directory else "") + base[:index] + base[index + 1:])
