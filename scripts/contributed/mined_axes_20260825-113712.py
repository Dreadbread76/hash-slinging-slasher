...
def main():
    ...
    ap.add_argument("--skip", type=int, default=0,
                    help="start this far down the ranking, skipping cuts already run")
    ...
    if args.classes:
        ...
        ranked = [(u, v, c) for (u, v), c in subs.most_common() if c >= args.min_seen]
        # `--skip` starts partway down the ranking. Lowering `--min-seen` opens thousands more
        # substitutions but re-emits everything above them, and regenerating ground already run
        # is pure Python time -- the expensive half of this method.
        kept = ranked[args.skip:args.skip + args.top]
    if args.report:
        for u, v, c in kept:
            print("%8d  %-8s -> %s" % (c, u, v))
        return
