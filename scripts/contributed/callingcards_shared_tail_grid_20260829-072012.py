"""Fill the measured callingcards family with tails shared by multiple axes."""
import collections
import pathlib
import sys
...
def main():
    ...
    for name in have:
        ...
        if head == "callingcards" and axis and tail:
    ...
    candidates = sorted({f"callingcards_{axis}_{tail}" for axis in axes for tail in tails} - have)
    print(f"callingcards shared-tail grid: {len(axes):,} axes x {len(tails):,} tails = "
