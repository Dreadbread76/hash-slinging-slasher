"""Fill the measured icon family using tails shared by multiple observed axes."""
import collections
import pathlib
import sys
...
def main():
    ...
    for name in have:
        ...
        if head == "icon" and axis and tail:
    ...
    candidates = sorted({f"icon_{axis}_{tail}" for axis in axes for tail in tails} - have)
    print(f"icon shared-tail grid: {len(axes):,} axes x {len(tails):,} tails = "
