...
This is narrower than the exhausted all-heads x all-tails splice: by default only tokens occurring
in 2--12 names of the same type are crossed. `--min-family` and `--max-family` allow disjoint
occurrence bands to be measured without changing the generator.
"""
import argparse
import collections
import os
...
def candidates(kind, minimum, maximum):
    known = names(kind)
    ...
    for token, family in sorted(by_token.items()):
        if not minimum <= len(family) <= maximum:
            continue
        pieces = []
        for name in family:
...
def main():
    ...
    parser.add_argument("--max-family", type=int, default=12)
    args = parser.parse_args()
    kinds = [args.kind] if args.kind else sorted(TABLES)
    all_candidates = set()
    for kind in kinds:
        current = set(candidates(kind, args.min_family, args.max_family))
