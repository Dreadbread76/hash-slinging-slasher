"""Combine triples by thirteenth power of rotated alphabet-index spread."""
import os, sys
        ...
        for _, c in seeds:
            ...
            for i in range(max(len(a),len(b),len(c))):
                ...
                if len(vals)==3 and all(v.isalpha() for v in vals):
                    nums=[rot(ord(v)-96) for v in vals]; spread=max(nums)-min(nums)
                    out.append(chr(97 + (spread**13)%26))
